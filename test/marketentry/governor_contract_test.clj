(ns marketentry.governor-contract-test
  ...
  test:
    ...
    OWN flagship invariant: the governor correctly identifies the
    Agency on Public Procurement of Goods, Works and Services as the
    procurement regulator and NEVER confuses it with the Agency for
    State Financial Control and Combating Corruption (a wholly separate
    anti-corruption oversight body), HARD-holding any proposal that
    conflates them."
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems TJK"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems TJK" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
;; ---- the central fabrication trap for this jurisdiction: procurement
;; regulator vs. anti-corruption agency ----

(deftest clean-assess-correctly-distinguishes-procurement-agency-from-anti-corruption-agency
  (testing "a clean jurisdiction/assess proposal keeps the Agency on Public Procurement (regulator) and the Agency for State Financial Control and Combating Corruption (anti-corruption, DIFFERENT body) separate -- settles as normal escalate/approve/commit, no conflation hold"
    (let [[db actor] (fresh)
      ...
      (let [r2 (approve! actor "t5")
        ...
        (is (= :commit (get-in r2 [:state :disposition])))
        (is (= "Agency on Public Procurement of Goods, Works and Services under the Government of the Republic of Tajikistan"
               (:procurement-regulator-authority assessment)))
        (is (re-find #"Combating Corruption" (:anti-corruption-agency-authority assessment)))
        (is (not= (:procurement-regulator-authority assessment) (:anti-corruption-agency-authority assessment))
            "the governor must never let these collapse into a single value")))))

(deftest conflated-procurement-agency-claim-is-held-and-unoverridable
  (testing "a jurisdiction/assess proposal that conflates the procurement regulator with the anti-corruption agency -> HARD hold, settles immediately, no interrupt"
    (let [[db actor] (fresh)
          res (exec-op actor "t6" {:op :jurisdiction/assess :subject "eng-1" :conflate-procurement-agency? true} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:procurement-agency-conflated} (-> (store/ledger db) first :basis)))
      (is (nil? (store/assessment-of db "eng-1")) "no conflated assessment written -- HARD violations never commit"))))

(deftest procurement-agency-never-confused-with-anti-corruption-agency-across-many-runs
  (testing "the governor's procurement-agency check correctly identifies the Agency on Public Procurement on every clean run, and never lets the anti-corruption agency substitute for it, across repeated runs"
    (let [[db actor] (fresh)]
      (dotimes [n 3]
        (let [tid (str "loop-" n)]
          (exec-op actor tid {:op :jurisdiction/assess :subject "eng-1"} operator)
          (approve! actor tid)))
      (let [assessment (store/assessment-of db "eng-1")]
        (is (re-find #"(?i)public procurement" (:procurement-regulator-authority assessment)))
        (is (not (re-find #"(?i)combating corruption" (:procurement-regulator-authority assessment)))
            "the regulator field must never itself resolve to the anti-corruption agency's name")))))

(deftest business-registration-missing-is-held-and-unoverridable
  (testing "missing Tax Committee Unified-state-register business registration -> HARD hold"
    (let [[db actor] (fresh)
...
(deftest tax-registration-unverified-is-held-and-unoverridable
  (testing "unverified Tax Code (2012) tax registration when required -> HARD hold"
    (let [[db actor] (fresh)
...
(deftest investment-registration-missing-is-held-and-unoverridable
  (testing "unverified State Committee on Investments (Law No. 2173) foreign-investment registration when required -> HARD hold"
    (let [[db actor] (fresh)
          _ (assess! actor "t10pre" "eng-6")
          _ (draft! actor "t10pre" "eng-6")
          res (exec-op actor "t10" {:op :filing/submit :subject "eng-6"} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:investment-registration-missing} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
...
(deftest every-decision-leaves-one-ledger-fact
  (testing "write-only-through-ledger: N operations -> N ledger facts (append-only, never rewritten)"
    (let [[db actor] (fresh)]
      ...
      (exec-op actor "c" {:op :jurisdiction/assess :subject "eng-1" :conflate-procurement-agency? true} operator)
...
(deftest interrupt-before-request-approval-pauses-the-actor
  (testing "the compiled graph's interrupt-before boundary actually pauses execution -- no record/ledger write happens until resume"
    (let [[db actor] (fresh)
          r1 (exec-op actor "t15" {:op :jurisdiction/assess :subject "eng-1"} operator)]
