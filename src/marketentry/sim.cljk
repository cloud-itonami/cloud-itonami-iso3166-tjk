(ns marketentry.sim
  ...
  procurement-agency-conflation check."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (TJK, clean) ==")
    ...
    (println "== jurisdiction/assess eng-1 AGAIN, conflating the procurement agency with the anti-corruption agency (HARD hold -- flagship check) ==")
    (println (exec-op actor "t2b" {:op :jurisdiction/assess :subject "eng-1" :conflate-procurement-agency? true} operator))
    ...
    (println "== filing/submit eng-4 (business-registration-missing -> HARD hold) ==")
    ...
    (println "== jurisdiction/assess eng-6 (sets up investment-registration-missing) ==")
    ...
    (println "== filing/submit eng-6 (investment-registration-missing -> HARD hold) ==")
